...
const CompletedRequestsList = ({ searchQuery }: IProps) => {
  const { getCompletedRequestedReviews } = useReviewsApi();

  const {
    data: completedReviewRequestData,
    isLoading: loadingGetCompletedReviewRequest,
  } = useQuery(
    ["completed requests review", searchQuery],
    () => getCompletedRequestedReviews(searchQuery),
    {
  ...
  return (
    <>
      {!loadingGetCompletedReviewRequest && completedReviewRequestData ? (
        completedReviewRequestData.scripts.length > 0 ? (
          <Table className="mt-4 bg-white rounded-md shadow-primary  py-5 xl:py-8 flex flex-col mb-16">
            ...
            <TableBody className="px-6 xl:px-14 overflow-hidden">
              {completedReviewRequestData.scripts.map((script) => (
